...
from src.modules.activity.domain.entities.activity import (
    ActivityLine,
    ActivitySummary,
    Contributor,
)
...
def _arrange(
    missions: list[MissionRecord], lines: dict[int, ActivityLine]
) -> tuple[tuple[ActivityLine, ...], tuple[ActivityLine, ...]]:
    """Folds packages into their project and sorts everything by weight."""
    packages: dict[int, list[ActivityLine]] = defaultdict(list)
    top: list[ActivityLine] = []
...
def _with_packages(line: ActivityLine, packages: list[ActivityLine]) -> ActivityLine:
    if not packages:
        return line
    return replace(line, packages=_heaviest_first(packages))


def _heaviest_first(lines: Iterable[ActivityLine]) -> tuple[ActivityLine, ...]:
