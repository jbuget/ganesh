  ...
  return (
    ...
    >
      ...
      {/*
        ...
        projet » — and rendering them while the figures are still zero would
        state the opposite of what is being loaded.
      */}
      {home.isLoading ? (
        ...
        <div className="grid items-start gap-6 lg:grid-cols-3">
          <div className="space-y-6 lg:col-span-2">
            ...
            <section>
              <h2 className="mb-3 text-sm font-semibold text-slate-900">Mes projets</h2>
          ...
          {/* The news column keeps to itself: it is read down, at one's own pace,
            while the left column is scanned. Mixing the two would make the
            reminders scroll past what one came to do.

            Framed like a kanban column, and tinted like one: a stack of cards
            read one after the other is the same object on both screens, and
            the tint is what tells the eye where the stack stops. */}
          <aside className="rounded-xl border border-slate-300 bg-slate-100">
              ...
              <div className="flex flex-col gap-2 px-2 pb-2">
                {home.updates.map((entry) => (
                  <UpdateFeedItem
                    ...
                    entry={entry}
                    now={now}
                    // Opened on its thread: the message one just read is what one
                    // is going there for.
