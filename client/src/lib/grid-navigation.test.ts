...
const GRID: NavigableGrid = {
  // Rows are named by mission and trade now; these carry none.
  rows: ["10:", "20:", "30:"],
...
describe("nextCell", () => {
  it("moves to the next day on the same row", () => {
    expect(
      nextCell(
        GRID,
        { projectId: 20, activityId: null, day: "2026-09-01" },
        "ArrowRight",
      ),
    ).toEqual({
      projectId: 20,
  ...
  it("moves to the previous day on the same row", () => {
    expect(
      nextCell(
        GRID,
        { projectId: 20, activityId: null, day: "2026-09-02" },
        "ArrowLeft",
      ),
    ).toEqual({
      projectId: 20,
  ...
  it("moves to the same day on the row below", () => {
    expect(
      nextCell(
        GRID,
        { projectId: 10, activityId: null, day: "2026-09-02" },
        "ArrowDown",
      ),
    ).toEqual({
      projectId: 20,
  ...
  it("moves to the same day on the row above", () => {
    expect(
      nextCell(GRID, { projectId: 30, activityId: null, day: "2026-09-02" }, "ArrowUp"),
    ).toEqual({
      projectId: 20,
      activityId: null,
      day: "2026-09-02",
  ...
  it("goes to the first open day of the row", () => {
    expect(
      nextCell(GRID, { projectId: 20, activityId: null, day: "2026-09-04" }, "Home"),
    ).toEqual({
      projectId: 20,
      activityId: null,
      day: "2026-09-01",
  ...
  it("goes to the last open day of the row", () => {
    expect(
      nextCell(GRID, { projectId: 20, activityId: null, day: "2026-09-01" }, "End"),
    ).toEqual({
      projectId: 20,
      activityId: null,
      day: "2026-09-05",
  ...
  it("stays put at the end of a row: a grid does not wrap", () => {
    expect(
      nextCell(
        GRID,
        { projectId: 20, activityId: null, day: "2026-09-05" },
        "ArrowRight",
      ),
    ).toBeNull();
  });
  ...
  it("stays put on the last row", () => {
    expect(
      nextCell(
        GRID,
        { projectId: 30, activityId: null, day: "2026-09-02" },
        "ArrowDown",
      ),
    ).toBeNull();
  });
  ...
  it("stays put on the first row", () => {
    expect(
      nextCell(GRID, { projectId: 10, activityId: null, day: "2026-09-02" }, "ArrowUp"),
    ).toBeNull();
  });
  ...
  it("answers nothing when the whole column is closed", () => {
    expect(
      nextCell(
        GRID,
        { projectId: 10, activityId: null, day: "2026-09-03" },
        "ArrowDown",
      ),
    ).toBeNull();
  });
  ...
  it("answers nothing for a cell the grid does not hold", () => {
    expect(
      nextCell(
        GRID,
        { projectId: 99, activityId: null, day: "2026-09-02" },
        "ArrowRight",
      ),
    ).toBeNull();
    expect(
      nextCell(
        GRID,
        { projectId: 10, activityId: null, day: "2026-10-01" },
        "ArrowRight",
      ),
    ).toBeNull();
  });
  ...
  it("answers nothing when every day of the row is closed", () => {
    ...
    expect(
      nextCell(closed, { projectId: 20, activityId: null, day: "2026-09-02" }, "Home"),
    ).toBeNull();
  });
});
...
describe("cellId", () => {
  it("names a cell and reads it back", () => {
    const cell = { projectId: 10, activityId: null, day: "2026-09-14" };
...
describe("tabStop", () => {
  it("is the first open cell before anybody has touched the grid", () => {
    expect(tabStop(GRID, null)).toEqual({
      projectId: 10,
      activityId: null,
      day: "2026-09-01",
    });
  ...
  it("falls back when the cursor sits on a day that closed", () => {
    expect(
      tabStop(GRID, { projectId: 20, activityId: null, day: "2026-09-03" }),
    ).toEqual({
      projectId: 10,
      activityId: null,
      day: "2026-09-01",
